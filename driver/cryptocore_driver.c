...
static void MWMAC_ECMDPointMUL(ECMDPointMUL_params_t *ECMDPointMUL_params_ptr);
...
{	
	...
	ECMDPointMUL_params_t *ECMDPointMUL_params_ptr = kmalloc(sizeof(ECMDPointMUL_params_t), GFP_DMA);	
	...
	switch(cmd) {
		...
		case IOCTL_MWMAC_ECMDPOINTADD:
			...
			break;	
		case IOCTL_MWMAC_ECMDPOINTMUL:
			rc = copy_from_user(ECMDPointMUL_params_ptr, (void *)arg, sizeof(ECMDPointMUL_params_t));
			MWMAC_ECMDPointMUL(ECMDPointMUL_params_ptr);
			rc = copy_to_user((void *)arg, ECMDPointMUL_params_ptr, sizeof(ECMDPointMUL_params_t));
			break;				
...
From here P. Addition, P.Doubling or P.Mult. can be done in different ways
	For P.Addition:	
		...
		// Jacobi Transform is done by just introducing extra z_MD, x_MD & y_MD remains same 
			...
			REGISTERS:
				B8, X8	<-- x_PMDJ		 CopyV2V X8 -> X5,B5
				B7, X7	<-- y_PMDJ		 CopyV2V X7 -> X4,B4
				B6,	X6	<-- z_PMDJ		 CopyV2V X6 -> X3,B3
...
just an idea
	...
	Point Multiplication are skipped for now..
		...
		ZR  = E7 <- ModSub(E3, E5, p)			 	 ModSub(TS1, TC1, P1)
		                ...
		                COPY V2V                        COPYV2V(E7, X6)  // UPDATE VALUES 	X6 <- z_rmdj_dbl 		                
						COPY	V2H					    COPYV2H(E7, B6)  // RESET VALUES 	B6 <- z_rmdj_dbl  

		                COPY V2V                        COPYV2V(E6, E5)  // YR = E5
		                COPY V2V                        COPYV2V(E5, X7)	 // UPDATE VALUES 	X7 <- y_rmdj_dbl 		
						COPY	V2H					    COPYV2H(E5, B7)	 // RESET VALUES 	B7 <- y_rmdj_dbl 		                					
		
		                COPY V2V                        COPYV2V(E8, E6)  // XR = E6  		
		                COPY V2V                        COPYV2V(E6, X8)  // UPDATE VALUES 	X8 <- X_rmdj_dbl  
						COPY	V2H					    COPYV2H(E6, B8)	 // RESET VALUES 	B8 <- X_rmdj_dbl 
					...
					REGISTERS (IMPORTANTS):
						...
						X8 <- X_rmdj_dbl 		X7 <- y_rmdj_dbl	 	X6 <- z_rmdj_dbl  	
						B8 <- X_rmdj_dbl 		B7 <- y_rmdj_dbl 	 	B6 <- z_rmdj_dbl  
...
Z_RMDJ	=	E7	<-	MontMult(E4, E1, p)				MontMult(E4, B1, P1)
						...
						COPY V2H					    COPYV2H(X5, B5)	 // RESET VALUES 	B5 <- x_pmdj 												
						COPY V2H					    COPYV2H(X4, B4)	 // RESET VALUES 	B4 <- y_pmdj 												
						COPY V2H					    COPYV2H(X3, B3)	 // RESET VALUES 	B3 <- z_pmdj 												
...
//x_pG		write	B8
	...
	//COPYH2V(B8,X5)
		...
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYH2V << 8) 
		//			src_addr      				dest_addr   	 src_addr_e   src_addr_x
				| (MWMAC_RAM_B8 << 12) | (MWMAC_RAM_X5 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
	...
	//COPYH2H(B8,B5)
		...
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYH2H << 8) 
		//			src_addr      				dest_addr   	 src_addr_e   src_addr_x
				| (MWMAC_RAM_B8 << 12) | (MWMAC_RAM_B5 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;		
...
//y_pG		write	B7
	...
	//COPYH2V(B7,X4)
		...
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYH2V << 8) 
		//			src_addr      				dest_addr   	 src_addr_e   src_addr_x
				| (MWMAC_RAM_B7 << 12) | (MWMAC_RAM_X4 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
	...
	//COPYH2H(B7,B4)
		...
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYH2H << 8) 
		//			src_addr      				dest_addr   	 src_addr_e   src_addr_x
				| (MWMAC_RAM_B7 << 12) | (MWMAC_RAM_B4 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;		
...
//z_p				
	...
	//COPYH2V(B6,X3)
		...
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYH2V << 8) 
		//			src_addr      				dest_addr   	 src_addr_e   src_addr_x
				| (MWMAC_RAM_B6 << 12) | (MWMAC_RAM_X3 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
	...
	//COPYH2H(B6,B3)
		...
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYH2H << 8) 
		//			src_addr      				dest_addr   	 src_addr_e   src_addr_x
				| (MWMAC_RAM_B6 << 12) | (MWMAC_RAM_B3 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;		
	...
	/*
			REGISTERS:
				B8, X8	<-- x_PMDJ		 CopyV2V B8 -> X5,B5
				B7, X7	<-- y_PMDJ		 CopyV2V B7 -> X4,B4
				B6,	X6	<-- z_PMDJ		 CopyV2V B6 -> X3,B3
			*/
...
//ZR = E7		
 	...
 	// UPDATE VALUES 	X6 <- z_rmdj_dbl
		//COPYV2V(E7, X6) 		 // UPDATE VALUES 	X6 <- z_rmdj_dbl
		//            Start     Abort       f_sel     sec_calc        precision         operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2V << 8) 
		//			src_addr      			dest_addr    		src_addr_e   	src_addr_x
				| (MWMAC_RAM_E7 << 12) | (MWMAC_RAM_X6 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
		
 	// RESET VALUES 	B6 <- z_rmdj_dbl				 		                
		//COPYV2H(E7, B6)		 // RESET VALUES 	B6 <- z_rmdj_dbl
		//            Start     Abort       f_sel     sec_calc        precision              operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2H << 8) 
		//			src_addr      			dest_addr    		src_addr_e   src_addr_x
				| (MWMAC_RAM_E7 << 12) | (MWMAC_RAM_B6 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
...
// YR = E5
		...
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
	 // UPDATE VALUES 	X7 <- y_rmdj_dbl 			
		//COPYV2V(E5, X7)	 	 // UPDATE VALUES 	X7 <- y_rmdj_dbl 
		//            Start     Abort       f_sel     sec_calc        precision         operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2V << 8) 
		//			src_addr      			dest_addr    		src_addr_e   	src_addr_x
				| (MWMAC_RAM_E5 << 12) | (MWMAC_RAM_X7 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
 	 // RESET VALUES 	B7 <- y_rmdj_dbl					
		//COPYV2H(E5, B7)		 // RESET VALUES 	B7 <- y_rmdj_dbl
		//            Start     Abort       f_sel     sec_calc        precision              operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2H << 8) 
		//			src_addr      			dest_addr    		src_addr_e   src_addr_x
				| (MWMAC_RAM_E5 << 12) | (MWMAC_RAM_B7 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
...
//XR = E6
		...
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
	 // UPDATE VALUES 	X8 <- X_rmdj_dbl  			  		
		//COPYV2V(E6, X8) 		 // UPDATE VALUES 	X8 <- X_rmdj_dbl  
		//            Start     Abort       f_sel     sec_calc        precision         operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2V << 8) 
		//			src_addr      			dest_addr    		src_addr_e   	src_addr_x
				| (MWMAC_RAM_E6 << 12) | (MWMAC_RAM_X8 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
	 // RESET VALUES	B8 <- X_rmdj_dbl		
		//COPYV2H(E6, B8)		 // RESET VALUES 	B8 <- X_rmdj_dbl
		//            Start     Abort       f_sel     sec_calc        precision              operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2H << 8) 
		//			src_addr      			dest_addr    		src_addr_e   src_addr_x
				| (MWMAC_RAM_E6 << 12) | (MWMAC_RAM_B8 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;
...
//UPDATE B REGISTERS					 			
  	  ...
  	  //COPYV2H(X5, B5)	 // RESET VALUES 	B5 <- x_pmdj
		//            Start     Abort       f_sel     sec_calc        precision              operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2H << 8) 
		//			src_addr      			dest_addr    		src_addr_e   src_addr_x
				| (MWMAC_RAM_X5 << 12) | (MWMAC_RAM_B5 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;		 
														
	  //COPYV2H(X4, B4)	 // RESET VALUES 	B4 <- y_pmdj
		//            Start     Abort       f_sel     sec_calc        precision              operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2H << 8) 
		//			src_addr      			dest_addr    		src_addr_e   src_addr_x
				| (MWMAC_RAM_X4 << 12) | (MWMAC_RAM_B4 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;	   	
													
	  //COPYV2H(X3, B3)	 // RESET VALUES 	B3 <- z_pmdj 												
		//            Start     Abort       f_sel     sec_calc        precision              operation
		mwmac_cmd = (1 << 0) | (0 << 1) | (mwmac_f_sel << 2) | (mwmac_sec_calc << 3) | (mwmac_cmd_prec << 4) | (COPYV2H << 8) 
		//			src_addr      			dest_addr    		src_addr_e   src_addr_x
				| (MWMAC_RAM_X3 << 12) | (MWMAC_RAM_B3 << 17) | (0x0 << 22) | (0x0 << 27);
		iowrite32(mwmac_cmd, MWMAC_CMD_ptr);
		while(!mwmac_irq_var);
		mwmac_irq_var = 0;								
...
/*						
	REGISTERS (IMPORTANTS):
			...
			X8 <- X_rmdj_add 		X7 <- y_rmdj_add	 	X6 <- z_rmdj_add  	
			B8 <- X_rmdj_add 		B7 <- y_rmdj_add 	 	B6 <- z_rmdj_add  
...
static void MWMAC_ECMDPointMUL(ECMDPointMUL_params_t *ECMDPointMUL_params_ptr)
{
	...
	if(ECMDPointMUL_params_ptr->f_sel == 0) {
		mwmac_f_sel = 0;
		for(i=0; i<16; i++){
			if(BINARY_PRECISIONS[i][0]==ECMDPointMUL_params_ptr->prec){
				mwmac_cmd_prec = BINARY_PRECISIONS[i][1];
				if(ECMDPointMUL_params_ptr->prec % 32) {
					rw_prec = (ECMDPointMUL_params_ptr->prec/32 + 1) * 32;
				} else {
					rw_prec = ECMDPointMUL_params_ptr->prec;
				}
			}
		}
	}
		...
		mwmac_f_sel = 1;
		for(i=0; i<13; i++){
			if(PRIME_PRECISIONS[i][0]==ECMDPointMUL_params_ptr->prec){
				mwmac_cmd_prec = PRIME_PRECISIONS[i][1];
				rw_prec = ECMDPointMUL_params_ptr->prec;
			}
		}
	...
	if(ECMDPointMUL_params_ptr->sec_calc == 0) {
		mwmac_sec_calc = 0;
	}
	else {
		mwmac_sec_calc = 1;
	...
	i= ioread32(TRNG_FIFO_ptr);
	i &= 0x7FFFFFFF;
	i |= 0x40000000;
	ECMDPointMUL_params_ptr->k[rw_prec/32-1] = 	i;
	for(i=1; i<rw_prec/32; i++){
		
		ECMDPointMUL_params_ptr->k[rw_prec/32-1-i] = ioread32(TRNG_FIFO_ptr);
	} 
